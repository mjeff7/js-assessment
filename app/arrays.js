arraysAnswers = {
  ...
  sum: function sum(arr) {
    return arr.reduce((accumlator, number) => accumlator + number, 0);
  },
  ...
  /**
   * Remove all instances of item from an array.
   *
   * @param {Number[]} arr - An array of numbers
   * @param {Number} item - A number to be excluded from the new array
  ...
  removeWithoutCopy: function removeWithoutCopy(arr, item) {
    // Empty the array and replace it with filtered contents. This is faster
    // than calling splice repeatedly when item appears many times.
    arr.splice(0, arr.length, ...arraysAnswers.remove(arr, item));
    return arr;
  },
  ...
  count: function count(arr, item) {
    return arr.reduce(
      (runningCount, arrItem) =>
        (arrItem === item ? runningCount + 1 : runningCount),
      0,
    );
  },
  ...
  findAllOccurrences: function findAllOccurrences(arr, target) {
    return arr.reduce(
      (foundIndices, arrItem, arrItemIndex) =>
        (arrItem === target ? foundIndices.concat(arrItemIndex) : foundIndices),
      [],
    );
  },
};
