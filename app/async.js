  ...
  manipulateRemoteData: function manipulateRemoteData(url) {
    return (
      fetch(url)
        ...
        .then(result => result.people.map(person => person.name).sort())
        // TODO: Handle errors more formally than logging to console.
        .catch(err =>
          console.err('Error caught in manipulateRemoteData: ', err),
        )
    );
  },
};
